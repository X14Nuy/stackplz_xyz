...
func FindLibInMaps(pid uint32, brk_lib string) (LibInfo, error) {
	...
	if libInfos, ok := pid_maps[brk_lib]; ok {
		return lowestMapping(libInfos), nil
	}

	var matchedPath string
	var matched []LibInfo
	for libPath, libInfos := range pid_maps {
		if len(libInfos) == 0 || libInfos[0].LibName != brk_lib {
			continue
		}
		if matchedPath != "" && matchedPath != libPath {
			return LibInfo{}, fmt.Errorf("library name %q is ambiguous between %q and %q", brk_lib, matchedPath, libPath)
		}
		matchedPath = libPath
		matched = libInfos
	}
	if matchedPath == "" {
		return LibInfo{}, fmt.Errorf("library %q not found in maps for pid %d", brk_lib, pid)
	}
	return lowestMapping(matched), nil
}

func lowestMapping(infos []LibInfo) LibInfo {
	lowest := infos[0]
	for _, info := range infos[1:] {
		if info.BaseAddr < lowest.BaseAddr {
			lowest = info
		}
	}
	return lowest
...
func LoadMapsFile(pid uint32, filename string) error {
	if pid == 0 {
		return errors.New("maps pid must be nonzero")
	}
	if filename == "" {
		return errors.New("maps filename is empty")
	}
	content, err := os.ReadFile(filename)
	if err != nil {
		return fmt.Errorf("read maps file %q: %w", filename, err)
	}

	if err := LoadMapsContent(pid, content); err != nil {
		return fmt.Errorf("parse maps file %q: %w", filename, err)
	}
	return nil
}

func LoadMapsContent(pid uint32, content []byte) error {
	if pid == 0 {
		return errors.New("maps pid must be nonzero")
	}
	if len(content) == 0 {
		return errors.New("maps content is empty")
	}
	temporary := NewMapsHelper()
	if err := temporary.ParseMapsContent(pid, content, true); err != nil {
		return fmt.Errorf("parse maps content: %w", err)
	}
	parsed := temporary.pid_maps[pid]
	if parsed == nil || len(*parsed) == 0 {
		return errors.New("maps content has no valid mappings")
	}
	replacement := parsed.Clone()
	maps_lock.Lock()
	maps_helper.pid_maps[pid] = &replacement
	maps_lock.Unlock()
	return nil
}
