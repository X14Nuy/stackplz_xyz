...
func persistentPreRunEFunc(command *cobra.Command, args []string) error {
	...
	requestedSource, err := ParseTargetSource(gconfig.TaskSource)
	if err != nil {
		return err
	}
	requestedBackend, err := ParseBreakBackend(gconfig.BrkBackend)
	if err != nil {
		return err
	}
	directKPMStartup := requestedBackend == BreakBackendKPMDirect
	if !directKPMStartup && !gconfig.NoCheck {
		// 先检查必要的配置
		err = util.CheckKernelConfig()
		if err != nil {
			return fmt.Errorf("CheckKernelConfig failed: %w", err)
		}
	}
	if directKPMStartup {
		mconfig.ExternalBTF = ""
	...
	if !directKPMStartup && gconfig.SdkInt == 0 {
	...
	source, requestedBackend, err := validateTracingOptions(gconfig, mconfig.PidWhitelist)
	if err != nil {
		return err
	}
	if source != requestedSource {
		return errors.New("task source changed during validation")
	}
	var controlClient *kpm.Client
	if source != TargetSourceProc || requestedBackend != BreakBackendPerf {
		controlClient, err = newKPMClient(gconfig)
		if err != nil {
			return fmt.Errorf("create KPM client: %w", err)
		}
	}
	selectedBackend, err := selectBreakBackend(requestedBackend, func() error {
		if controlClient == nil {
			return errors.New("KPM client is unavailable")
		}
		_, statusErr := controlClient.Status(command.Context())
		if statusErr != nil {
			logger.Printf("KPM readiness check failed: %v", statusErr)
		}
		return statusErr
	})
	if err != nil {
	...
	gconfig.BrkBackend = string(selectedBackend)
	logger.Printf("selected task source=%s breakpoint backend=%s", source, selectedBackend)

	originalUIDs := append([]uint32(nil), mconfig.UidWhitelist...)
	var uidOverride *uint32
	if len(originalUIDs) == 1 {
		value := originalUIDs[0]
		uidOverride = &value
	} else if len(originalUIDs) > 1 && (source == TargetSourceKPM || selectedBackend == BreakBackendKPMDirect) {
		return errors.New("KPM target resolution accepts at most one --uid")
	}
	resolver := &liveTargetResolver{
		client:       controlClient,
		uidOverride:  uidOverride,
		bindTimeout:  gconfig.KPMBindTimeout,
		pollInterval: 10 * time.Millisecond,
	}
	var pis *util.PackageInfos
	getPackageInfos := func() *util.PackageInfos {
		if pis == nil {
	...
	var procPIDs []uint32
	resolvedTargets := make([]ResolvedTarget, 0, len(mconfig.PidWhitelist))
	kpmMapsErrors := make(map[uint32]error)
	if len(mconfig.PidWhitelist) > 0 {
		resolvedTargets, err = resolveRuntimePIDTargets(command.Context(), resolver, source, mconfig.PidWhitelist, uidOverride, func(target ResolvedTarget) error {
			procPIDs = append(procPIDs, target.PID)
			is_find, info := getPackageInfos().FindPackageByUid(target.UID)
			if !is_find {
				logger.Printf("can not find package for process_pid=%d", target.PID)
				return nil
			}
			addLibPath(info.Name)
			search_paths, findErr := event.FindLibPaths(target.PID)
			if findErr != nil {
				return findErr
			}
		...
		kpmMapsErrors = cacheKPMTargetMaps(command.Context(), controlClient,
			resolvedTargets, gconfig.MapsFile)
		for _, target := range resolvedTargets {
			if target.Source != TargetSourceKPM {
				continue
			}
			if mapsErr, failed := kpmMapsErrors[target.PID]; failed {
				logger.Printf("KPM maps unavailable for pid %d: %v", target.PID,
					mapsErr)
				continue
			}
			searchPaths, findErr := event.FindLibPaths(target.PID)
			if findErr != nil {
				kpmMapsErrors[target.PID] = findErr
				logger.Printf("KPM maps library paths unavailable for pid %d: %v",
					target.PID, findErr)
				continue
			}
			for _, searchPath := range searchPaths {
				if !slices.Contains(gconfig.LibraryDirs, searchPath) {
					gconfig.LibraryDirs = append(gconfig.LibraryDirs, searchPath)
				}
			}
		}
		if err := clearKPMPreparationBinding(command.Context(), controlClient, resolvedTargets); err != nil {
			return err
		}
		for _, target := range resolvedTargets {
			if !slices.Contains(mconfig.UidWhitelist, target.UID) {
				mconfig.UidWhitelist = append(mconfig.UidWhitelist, target.UID)
			}
		}
	}
	// 根据 uid 解析进程架构、获取库文件搜索路径
	for _, pkg_uid := range originalUIDs {
		...
		is_find, info := getPackageInfos().FindPackageByUid(pkg_uid)
		if !is_find {
	...
	for _, pkg_name := range pkg_names {
		...
		default:
			is_find, info := getPackageInfos().FindPackageByName(pkg_name)
			if !is_find {
				return fmt.Errorf("can not find pkg_name=%s", pkg_name)
			}
			// 根据包名查找进程名 再获取库搜索路径
			pid_list := FindPidByName(pkg_name)
			procPIDs = append(procPIDs, pid_list...)
	...
	if len(procPIDs) > 0 {
		if err := util.SaveMaps(procPIDs); err != nil {
			return err
		}
	}
	...
	if selectedBackend != BreakBackendKPMDirect {
		// 1. load config file
	...
	// 4. watch breakpoint
	var brk_base uint64 = gconfig.BrkBase
	if gconfig.BrkLib != "" {
		targetPID, err := singleBreakpointPID(mconfig.PidWhitelist)
		if err != nil {
			return err
		}
		effectiveSource := source
		for _, target := range resolvedTargets {
			if target.PID == targetPID {
				effectiveSource = target.Source
				break
			}
		}
		if mapsErr, failed := kpmMapsErrors[targetPID]; failed &&
			effectiveSource == TargetSourceKPM && gconfig.MapsFile == "" &&
			gconfig.BrkBase == 0 {
			return fmt.Errorf("acquire KPM maps for --brk-lib pid %d: %w",
				targetPID, mapsErr)
		}
		brk_base, err = resolveBreakpointBaseForSource(effectiveSource, targetPID, gconfig.BrkLib, gconfig.MapsFile, gconfig.BrkBase)
		if err != nil {
			return err
		}
	}
	...
	if !enable_hook {
		return errors.New("hook nothing, plz set -w/--point or -s/--syscall or --brk")
	}
...
func runEFunc(command *cobra.Command, args []string) error {
	stopper := make(chan os.Signal, 1)
	signal.Notify(stopper, os.Interrupt, syscall.SIGTERM)
	defer signal.Stop(stopper)

	ctx, cancelFun := context.WithCancel(command.Context())
	defer cancelFun()
	if gconfig.Rpc {
	...
	modNames, err := selectModuleNames(mconfig)
	if err != nil {
		return err
	}
	defer mconfig.DumpClose()
	runModules, err := startModules(ctx, Logger, mconfig, modNames, registeredModule)
	if err != nil {
		return err
	}
	Logger.Printf("start %d modules", len(runModules))
	...
	cancelFun()
	if err := closeModules(runModules); err != nil {
		return err
	}
	for _, mod := range runModules {
		Logger.Printf("%s module closed", mod.Name())
	}
	return nil
}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&gconfig.TaskSource, "task-source", gconfig.TaskSource, "target lookup source: proc, kpm, or auto")
	rootCmd.PersistentFlags().StringVar(&gconfig.BrkBackend, "brk-backend", gconfig.BrkBackend, "breakpoint backend: perf, kpm-direct, or auto")
	rootCmd.PersistentFlags().StringVar(&gconfig.KPMProfile, "kpm-profile", gconfig.KPMProfile, "exact KPM device profile id")
	rootCmd.PersistentFlags().StringVar(&gconfig.KPMControl, "kpm-control", gconfig.KPMControl, "KPatch control binary path")
	rootCmd.PersistentFlags().StringVar(&gconfig.KPMModule, "kpm-module", gconfig.KPMModule, "loaded KPM module name")
	rootCmd.PersistentFlags().StringVar(&gconfig.MapsFile, "maps-file", "", "offline proc maps snapshot")
	rootCmd.PersistentFlags().Uint64Var(&gconfig.BrkBase, "brk-base", 0, "explicit breakpoint library base address")
	rootCmd.PersistentFlags().StringVar(&gconfig.BrkMode, "brk-mode", gconfig.BrkMode, "direct breakpoint mode: once or repeat")
	rootCmd.PersistentFlags().DurationVar(&gconfig.KPMBindTimeout, "kpm-bind-timeout", gconfig.KPMBindTimeout, "scheduler identity bind timeout")
	// 缓冲区大小设定 单位M
